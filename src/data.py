# Ref: https://huggingface.co/datasets/daily_dialog/raw/main/daily_dialog.py
# coding=utf-8
            ...
            features=datasets.Features(
                {
                    ...
                    "act": datasets.features.Sequence(datasets.ClassLabel(names=list(act_label.keys()))),
                    "emotion": datasets.features.Sequence(datasets.ClassLabel(names=list(emotion_label.keys()))),
                }
            ),
    ...
    def _split_generators(self, dl_manager: datasets.DownloadManager):
        ...
        return [
            datasets.SplitGenerator(
                name=split,
                gen_kwargs={
                    "data_zip": os.path.join(self.data_dir, f"{split}.zip"),
                ...
                with zip_file.open(dialog_path) as dialog_file, zip_file.open(act_path) as act_file, zip_file.open(
                    ...
                    for idx, (dialog_line, act_line, emotion_line) in enumerate(
                        ...
                        for i in range(2, len(dialog)):    
                            yield f"{idx}.{i}", {
                                "dialog": dialog[:i],
                                "act": [int(x) for x in act[:i]],
                                "emotion": [int(x) for x in emotion[:i]]
                            }

if __name__ == "__main__":

    data = DailyDialog("./ijcnlp_dailydialog")
    print(data)